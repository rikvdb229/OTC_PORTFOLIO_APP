...
class UndoRedoManager {
  ...
  async takeInitialSnapshot() {
    try {
      const snapshot =
        await window.IPCCommunication.Database.exportDatabaseData();
      if (snapshot.success) {
  ...
  async takeSnapshot(operationName) {
    ...
    try {
      ...
      console.log(
        `✅ Snapshot taken: ${operationName} (${this.undoStack.length} in stack)`
      );
      return true;
    } catch (error) {
  ...
  async undo() {
    ...
    try {
      ...
      console.log(
        ...
        undoEntry.snapshot?.portfolioEntries?.length
      );

      // Restore previous state
      const importResult =
        await window.IPCCommunication.Database.importDatabaseFromData(
          undoEntry.snapshot,
          false // replace mode
        );
      ...
      if (importResult.success) {
        ...
        this.showNotification(
          "Undo failed: " + (importResult.error || "Unknown error"),
          "error"
        );
      }
    } catch (error) {
  ...
  async redo() {
    ...
    try {
      ...
      console.log(
        "🔍 Redo entry snapshot size:",
        JSON.stringify(redoEntry.snapshot).length
      );
      ...
      console.log(
        "📊 Portfolio entries count:",
        redoEntry.snapshot?.portfolioEntries?.length
      );
      ...
      if (importResult.success) {
        ...
        this.showNotification(
          "Redo failed: " + (importResult.error || "Unknown error"),
          "error"
        );
      }
    } catch (error) {
        ...
        case "sales-history":
          if (this.app && this.app.loadSalesHistory) {
            ...
            console.log(
              ...
              Object.getOwnPropertyNames(this.app).filter((name) =>
                name.includes("load")
              )
            );
          }
